...
static bool retain_ref(atomic_uint *ref) {
    ...
    while (n && n != UINT_MAX) {
        if (atomic_compare_exchange_weak_explicit(ref, &n, n + 1,
...
bool IHS_FrameTrackerOpenEpoch(IHS_FrameTracker *t, uint64_t epoch) {
    ...
    if (ok) {
        ...
        for (size_t i = 0; i < IHS_FRAME_TRACKER_CAPACITY; ++i) t->seenValid[i] = false;
    }
...
void IHS_FrameTrackerCloseEpoch(IHS_FrameTracker *t, uint64_t epoch) {
    IHS_MutexLock(t->lock);
    if (epoch == t->epoch) close_epoch(t);
    IHS_MutexUnlock(t->lock);
}
...
bool IHS_FrameTicketRetain(IHS_FrameTicket *r) {
    if (!retain_ref(&r->owner->refs)) return false;
    if (retain_ref(&r->refs)) return true;
...
IHS_FrameIdentity IHS_FrameTicketIdentity(const IHS_FrameTicket *r) { return r->frame.identity; }
...
bool IHS_FrameTicketComplete(IHS_FrameTicket *r, const IHS_FrameOutcome *outcome) {
    if (!outcome || outcome->result <= IHS_VideoFrameResultPending ||
        ...
        (outcome->result == IHS_VideoFrameResultDisplayed && !outcome->presentationSerial)) return false;
    if (!IHS_FrameTicketClaim(r)) return false;
    IHS_FrameTicketPublish(r, outcome);
    return true;
}
IHS_FrameBeginResult IHS_FrameTrackerBegin(IHS_FrameTracker *t, uint64_t epoch,
    uint16_t frameId, const IHS_FrameReceive *receive, IHS_FrameTicket **out) {
    *out = NULL;
    IHS_MutexLock(t->lock);
    IHS_FrameBeginResult result = IHS_FrameBeginOK;
    if (!t->accepting || epoch != t->epoch) { result = IHS_FrameBeginClosed; goto done; }
    ...
    if (t->haveSequence) {
        ...
        if (delta == 32768) { result = IHS_FrameBeginAmbiguous; goto done; }
        if (delta < 32768) {
            if (UINT64_MAX - t->highestSequence < delta) { result = IHS_FrameBeginExhausted; goto done; }
            seq = t->highestSequence + delta;
        } else {
            unsigned back = 65536u - delta;
            if (back >= IHS_FRAME_TRACKER_CAPACITY || back > t->highestSequence) {
                result = IHS_FrameBeginStale; goto done;
            }
    ...
    for (size_t i = 0; i < IHS_FRAME_TRACKER_CAPACITY; ++i) {
        IHS_FrameTicket *r = &t->records[i];
        if (r->occupied && r->frame.identity.epoch == epoch && r->frame.identity.wireSequence == seq) {
            if (r->settled || atomic_load_explicit(&r->publication, memory_order_acquire) != OPEN) {
                result = IHS_FrameBeginStale; goto done;
            }
            if (!retain_ref(&t->refs)) { result = IHS_FrameBeginExhausted; goto done; }
            ...
            do {
                if (refs == UINT_MAX) { release_endpoint(t); result = IHS_FrameBeginExhausted; goto done; }
            } while (!atomic_compare_exchange_weak_explicit(&r->refs, &refs, refs + 1,
        ...
        if (!freeRecord && (!r->occupied || r->settled) &&
            ...
            r->frame.identity.slotVersion != UINT64_MAX) freeRecord = r;
    }
    size_t seenIndex = seq % IHS_FRAME_TRACKER_CAPACITY;
    if (t->seenValid[seenIndex] && t->seen[seenIndex] == seq) { result = IHS_FrameBeginStale; goto done; }
    if (!freeRecord) { result = IHS_FrameBeginCapacity; goto done; }
    if (t->serial == UINT64_MAX || !retain_ref(&t->refs)) { result = IHS_FrameBeginExhausted; goto done; }
    ...
    if (!t->haveSequence || seq > t->highestSequence) t->highestSequence = seq;
    t->haveSequence = true;
    *out = freeRecord;
 done:
    IHS_MutexUnlock(t->lock);
...
bool IHS_FrameTrackerDecodeStage(IHS_FrameTracker *t, IHS_FrameTicket *r, bool end, uint64_t us) {
    ...
    if (ok) {
        if (end) { r->frame.decodeEndUs = us; r->frame.hasDecodeEnd = true; }
        else { r->frame.decodeBeginUs = us; r->frame.hasDecodeBegin = true; }
    }
    IHS_MutexUnlock(t->lock);
    return ok;
}
size_t IHS_FrameTrackerSettle(IHS_FrameTracker *t, uint64_t nowUs, uint64_t timeoutUs,
    ...
    for (size_t i = 0; i < IHS_FRAME_TRACKER_CAPACITY && count < capacity; ++i) {
        IHS_FrameTicket *r = &t->records[i];
        if (!r->occupied || r->settled) continue;
        ...
        if (state == OPEN && nowUs >= r->frame.receive.firstReceiveUs &&
            ...
            if (atomic_compare_exchange_strong_explicit(&r->publication, &state, EXPIRED,
                memory_order_acq_rel, memory_order_acquire)) state = EXPIRED;
        }
        if (state != READY && state != EXPIRED) continue;
        out[count] = r->frame;
        out[count++].outcome = state == READY ? r->payload : (IHS_FrameOutcome){
            .result = IHS_VideoFrameResultDroppedLate, .completionUs = nowUs};
        r->settled = true;
    }
    IHS_MutexUnlock(t->lock);
    return count;
