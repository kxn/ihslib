...
static bool SessionChannelQueueFramePackets(IHS_SessionChannel *channel, IHS_SessionFrame *frame,
                                            size_t bodyLimit, bool enableRetransmit);
...
void IHS_SessionChannelAdd(IHS_Session *session, IHS_SessionChannel *channel) {
    ...
    if (existing == channel)
        return;
    if (existing ||
        session->numChannels >= sizeof(session->channels) / sizeof(session->channels[0])) {
...
void IHS_SessionChannelRemove(IHS_Session *session, IHS_SessionChannelId channelId) {
    if (channelId < IHS_SessionChannelIdDataStart)
        return;
    int channelIndex = -1;
    for (int i = 0; i < session->numChannels; ++i) {
        IHS_SessionChannel *channel = session->channels[i];
...
bool IHS_SessionChannelQueueFrame(IHS_SessionChannel *channel, IHS_SessionFrame *frame,
    ...
    if (frame->body.size < packetBodySizeLimit) {
        ...
        ret =
            SessionChannelQueueFramePackets(channel, frame, packetBodySizeLimit, enableRetransmit);
    }
    return ret;
}
